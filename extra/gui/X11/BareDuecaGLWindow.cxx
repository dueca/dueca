...
BareDuecaGLWindow::BareDuecaGLWindow(const char *window_title,
                                     bool pass_pointer, bool pass_keys,
                                     bool pass_passive, bool pointer_visible,
  ...
  pass_touch(pass_touch)
{
  eventmask =
    ...
    (pass_pointer ? ButtonPressMask | ButtonReleaseMask | ButtonMotionMask
                  : 0) |
...
{
  std::vector<int> wpos;
  wpos.push_back(width);
  wpos.push_back(height);
  wpos.push_back(posx);
  wpos.push_back(posy);
  setWindow(wpos);
}
...
void BareDuecaGLWindow::openWindow()
{
  ...
  GLint att[] = { GLX_RGBA,   GLX_DEPTH_SIZE,      24, GLX_DOUBLEBUFFER,
                  GLX_USE_GL, GLX_RED_SIZE,        8,  GLX_GREEN_SIZE,
                  8,          GLX_BLUE_SIZE,       8,  GLX_ALPHA_SIZE,
                  8,          GLX_ACCUM_RED_SIZE,  8,  GLX_ACCUM_GREEN_SIZE,
                  8,          GLX_ACCUM_BLUE_SIZE, 8,  GLX_ACCUM_ALPHA_SIZE,
                  8,          GLX_AUX_BUFFERS,     4,  None };
  XVisualInfo *vi = glXChooseVisual(static_dpy, screen, att);
  if (vi == NULL) {
    // try with less options
    GLint att[] = { GLX_RGBA,   GLX_DEPTH_SIZE,
                    24,         GLX_DOUBLEBUFFER,
                    GLX_USE_GL, GLX_RED_SIZE,
                    8,          GLX_GREEN_SIZE,
                    8,          GLX_BLUE_SIZE,
                    8,          GLX_ALPHA_SIZE,
                    8,          None };
  ...
  if (vi == NULL) {
    // try with less options
    GLint att[] = { GLX_RGBA,         GLX_DEPTH_SIZE, 16,
                    GLX_DOUBLEBUFFER, GLX_USE_GL,     None };
    vi = glXChooseVisual(static_dpy, screen, att);
  }
  ...
  if (fullscreen) {
    ...
    offset_x = rootWinAttr.x;
    offset_y = rootWinAttr.y;
    width = rootWinAttr.width;
    height = rootWinAttr.height;
  }
  my->xwin = XCreateWindow(static_dpy, xroot, offset_x, offset_y, width, height,
  ...
  if (fullscreen) {
    ...
    Atom wm_fullscreen =
      XInternAtom(static_dpy, "_NET_WM_STATE_FULLSCREEN", true);
  ...
  if (!keep_pointer) {
    ...
    Pixmap cursor_pixmap =
      XCreateBitmapFromData(static_dpy, my->xwin, emptycursor_bits, 1, 1);
    XColor black;
  ...
  if (pass_touch) {
    ...
    int ndevices = 0;
    XIDeviceInfo *devinfo = XIQueryDevice(static_dpy, XIAllDevices, &ndevices);
      ...
      if (devinfo[idev].use == XIFloatingSlave) {
        ...
        for (int ic = 0; ic < devinfo[idev].num_classes; ic++) {
          if (any[ic]->type == XITouchClass) {
            uint64_t mask =
              (1 << XI_TouchBegin) | (1 << XI_TouchUpdate) | (1 << XI_TouchEnd);
            XIEventMask emask = { devinfo[idev].deviceid, sizeof(mask),
                                  reinterpret_cast<unsigned char *>(&mask) };
            int stat = XISelectEvents(static_dpy, my->xwin, &emask, 1);
            switch (stat) {
            case BadValue:
              /* DUECA extra.
  ...
  my->glXWaitVideoSyncSGI =
    reinterpret_cast<PFNGLXWAITVIDEOSYNCSGIPROC>(glXGetProcAddress(
  ...
  PFNGLXSWAPINTERVALEXTPROC glXSwapIntervalEXT =
    reinterpret_cast<PFNGLXSWAPINTERVALEXTPROC>(glXGetProcAddress(
...
void BareDuecaGLWindow::redraw()
{
  ...
  for (;;) {
    Bool res = XCheckWindowEvent(static_dpy, my->xwin, eventmask, &u.x_event);
    if (res == False)
      break;
    switch (u.x_event.type) {
    case KeyRelease: {
      unsigned long kcode =
        XkbKeycodeToKeysym(static_dpy, u.x_event.xkey.keycode, 0, 0);
      if (kcode < 0xff) {
...
void BareDuecaGLWindow::waitSwap()
{
  if (my->glXWaitVideoSyncSGI) {
    ...
    unsigned int count;
    int res =
      (*(my->glXWaitVideoSyncSGI))(glx_sync_divisor, glx_sync_offset, &count);
    ...
    case GLX_BAD_CONTEXT:
      ...
      break;
    default:
      cerr << "error glXWaitVideoSyncSGI, unknown return value " << res << endl;
    }
