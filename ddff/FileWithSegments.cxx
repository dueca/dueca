...
namespace msgpack {
  ...
  MSGPACK_API_VERSION_NAMESPACE(v1) {
    ...
    namespace adaptor {
      ...
      template <>
      struct pack<dueca::ddff::FileWithSegments::Tag> {
                         ...
                         dueca::ddff::FileWithSegments::Tag& e)
{
  ...
  std::string tstring; msg_unpack(i0, iend, tstring);
...
ddff::FileHandler::pos_type
FileWithSegments::findOffset(unsigned cycle,
                             unsigned stream_id)
{
...
void FileWithSegments::bufferWriteInformation(pos_type offset,
                                         DDFFMessageBuffer::ptr_type buffer)
{
  ...
  if (buffer->object_offset && buffer->stream_id >= 2 &&
    ...
    next_tag.offset[buffer->stream_id - 2] = offset +
      std::ios::pos_type(buffer->object_offset);
  }
}
...
bool FileWithSegments::completeStretch(TimeTickType tick)
{
  ...
  if (ts_switch.validity_start == MAX_TIMETICK) return true;
  ...
  msgpack::packer<ddff::FileStreamWrite> pk(*w_tags); pk.pack(next_tag);
...
void FileWithSegments::replayLoad()
{
  for (auto &rdr: streams) {
    if (rdr.reader.get()) { rdr.reader->preload(); }
  }
}
...
void FileWithSegments::nameRecording(const std::string& label, const std::string& aux)
{
                            ...
                            FileWithSegments *ptr)
{
  ...
  if (ptr || !create_if_not_found) return NULL;
