...
bool DusimeControllerGtk::isPrepared()
{
  if (inactive == NULL && use_gui) {
    ...
    if (!inactive || !holdcurrent || !calibrate || !advance || !replay) {
      /* DUSIME UI.
    ...
    GladeCallbackTable cb_links[] = {
      { "inactive", "clicked", gtk_callback(&DusimeControllerGtk::cbInactive) },
      { "holdcurrent", "clicked",
        gtk_callback(&DusimeControllerGtk::cbHoldCurrent) },
      { "hw_calibrate", "clicked",
        gtk_callback(&DusimeControllerGtk::cbCalibrate) },
      { "replay", "clicked", gtk_callback(&DusimeControllerGtk::cbReplay) },
      { "advance", "clicked", gtk_callback(&DusimeControllerGtk::cbAdvance) },
      { "snap", "clicked", gtk_callback(&DusimeControllerGtk::cbSnapShot) },
      { NULL, NULL, NULL, NULL }
    };
...
void DusimeControllerGtk::refreshButtonState(const SimulationState &btn_state)
{
  ...
  int idx_sens = (not calibrated and btn_state == SimulationState::HoldCurrent)
                   ? int(SimulationState::Undefined) + 1
                   : int(btn_state.get());
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(inactive),
                               button_active[int(btn_state.get())][0]);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(holdcurrent),
                               button_active[int(btn_state.get())][1]);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(calibrate),
                               button_active[int(btn_state.get())][2]);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(advance),
                               button_active[int(btn_state.get())][3]);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(replay),
                               button_active[int(btn_state.get())][4]);
...
void DusimeControllerGtk::cbReplay(GtkWidget *widget, gpointer user_data)
{
  this->controlModel(SimulationState::Replay);
}

void DusimeControllerGtk::cbCalibrate(GtkWidget *widget,
                                          gpointer user_data)
{
...
void DusimeControllerGtk::cbInactive(GtkWidget *widget,
                                         gpointer user_data)
{
  // try the state change. If good, set the button to transitioning
...
void DusimeControllerGtk::cbHoldCurrent(GtkWidget *widget,
                                            gpointer user_data)
{
  this->controlModel(SimulationState::HoldCurrent);
}

void DusimeControllerGtk::cbAdvance(GtkWidget *widget,
                                        gpointer user_data)
{
  // try the state change. If good, set the button to transitioning
