...
#ifndef TriggerRegulator_hxx
#define TriggerRegulator_hxx
...
/** Make triggering (more) regular.
    ...
    A TriggerRegulator accepts a trigger and a time specification.
    Using the time specification, the triggering will be made
    regular. This only works with incoming spans. If a span starts
    (after a gap in data on a channel for example), one shorter
    ...
    myActivity.setTrigger(
      ...
      new TriggerRegulator(my_token,TimeSpec(0,20), 0)));
    \endcode
    ...
    This TriggerRegulator is not greedy, i.e., it will wait until the
    whole incoming span is present before providing another trigger.
    dueca::TriggerRegulatorGreedy will do greedy triggering, meaning
    that it will provide triggering as soon as a part of a new time
    stretch is present.
 */
class TriggerRegulator: public TargetAndPuller
{
private:
  ...
  TriggerRegulator(const TriggerRegulator&);
...
public:
  ...
  */
  TriggerRegulator(TriggerPuller& base, const TimeSpec& ts =
                   TimeSpec(0,1));
  ...
  /** Constructor with a ref-counter object as trigger
      ...
      @param ts       Time offset and period for triggering
  */
  TriggerRegulator(boost::intrusive_ptr<TargetAndPuller> base,
