...
public:
  ...
  */
  ...
  varvector(const varvector<T>& other) :
    N(other.size()), d(N == 0 ? NULL: new T[N])
  { for (int ii = N; ii--; ) this->d[ii] = other.d[ii]; }

  /** construct from iterators */
  template<class InputIt>
  varvector(InputIt first, InputIt last) : N(0)
  {
    for (auto firstc = first; firstc != last; firstc++) N++;
    d = N ? new T[N] : NULL;
  ...
  inline varvector<T>& operator = (const varvector<T>& other)
  {
    ...
    for (int ii = N; ii--; ) this->d[ii] = other[ii];
    return *this;
  }
  ...
  inline bool operator == (const varvector<T>& other) const
  {
    ...
    for (int ii = N; ii--; )
      if (this->d[ii] != other[ii]) return false;
    return true;
  }
  ...
  inline void resize(size_t s, const value_type& val = value_type())
  {
    if (s != N) {
      T* tmp = d;
      d = (s == 0) ? NULL : new T[s];
      for (size_t ii = std::min(s, N); ii--; ) d[ii] = tmp[ii];
      if (s > N) for (size_t ii = N; ii < s; ii++) d[ii] = val;
      N = s;
      delete [] tmp;
    }
  }
