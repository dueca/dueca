...
static GuiHandler* selectGuiHandler(const string& guiname)
{
  ...
  cerr << "Do not have graphic interface \"" << guiname
       << "\" linked in" << endl;
  std::exit(1);
}
...
Environment::Environment() :
  NamedObject(NameSet("dueca", "Environment",
...
{
  {
    ...
    double f1, f2;
    {
      ...
      tester << "1,2"; tester >> f1;
    }
    {
      std::stringstream tester;
      tester.imbue(std::locale(setlocale(LC_NUMERIC, NULL)));
      tester << "1.2"; tester >> f2;
    }
    if (f1 != 1.2 && f2 == 1.2) {
      // as expected
    }
  ...
  if (!checkGuiName(graphic_interface)) {
    ...
    */
    E_CNF("Gui name \"" << graphic_interface << "\" not known");
    return false;
  }
  ...
  if (highest_priority < 0 ||
    ...
    */
    E_CNF(getId() << "environment, priority specification error");
    return false;
  }

  // complete priority levels if only highest prio specified
  for (int p = 1; p <= highest_priority; p++) {
    sched_priorities.push_back
      (SchedPriority(SCHED_FIFO, sched_get_priority_min(SCHED_FIFO) +
                     PRIORITY_BASE + p));
  }
  highest_priority = sched_priorities.size() - 1;
  ...
  wait_additional = new ActivityCallback
    (getId(), "wait additional model copy", &cbw, PrioritySpec(0,0));

  return true;
}

const char* Environment::getTypeName()
{
  return "Environment";
}
...
const ParameterTable* Environment::getParameterTable()
{
  static const ParameterTable table[] = {
    { "multi-thread", new MemberCall<Environment,bool>
      (&Environment::setMultiThread),
      "(#t or #f), determines whether to use multi-threading (deprecated)!" },
    { "highest-priority", new VarProbe<Environment,int>
      (REF_MEMBER(&Environment::highest_priority)),
      ">=0, highest-priority activity manager (also deprecated)" },
    { "dummy-value", new VarProbe<Environment,int>
      (REF_MEMBER(&Environment::rt_mode)),
      "dummy, for compatibility with old .cnf files" },
    { "graphic-interface" , new VarProbe<Environment,vstring>
    ...
    { "graphic-use-depth-buffer", new MemberCall<Environment,bool>
      (&Environment::setGraphicUseDepthBuffer),
      "take a depth buffer with default size (16bits)" },
    { "graphic-depth-buffer-size", new VarProbe<Environment,int>
      (REF_MEMBER(&Environment::graphic_depth_buffer_size)),
      "required size of the depth buffer, default=16" },
    { "graphic-stencil-buffer-size", new VarProbe<Environment,int>
      (REF_MEMBER(&Environment::graphic_stencil_buffer_size)),
      "required size of the stencil buffer, default=4" },
    { "run-mode", new MemberCall<Environment,vstring>
      (&Environment::setRunMode),
    ...
    { "command-lead",
      new VarProbe<Environment,double>
      (REF_MEMBER(&Environment::command_lead)),
      "Lead time (in sec) needed for module start and stop commands,\n"
      "default is 0.1." },
    { "locale",
      new VarProbe<Environment,std::string>
      (&Environment::env_locale),
...
{
  if (y) graphic_depth_buffer_size = 16;
  else graphic_depth_buffer_size = 0;
  return true;
}
...
bool Environment::setAMNice(const vector<int>& level)
{
  for (unsigned int ii = 0; ii < level.size(); ii++) {
    if (level[ii] < -20 || level[ii] > 20) {
	 ...
	 affected node. */
      E_CNF(getId() << "environment, illegal nice level " << level[ii]);
      return false;
    }
...
bool Environment::setAMRoundRobin(const vector<int>& level)
{
  for (unsigned int ii = 0; ii < level.size(); ii++) {
    if (level[ii] < sched_get_priority_min(SCHED_RR) ||
      ...
      */
      E_CNF(getId() << "environment, illegal RR priority " << level[ii]);
      return false;
    }
  ...
  for (unsigned int ii = 0; ii < level.size(); ii++) {
    if (level[ii] < sched_get_priority_min(SCHED_FIFO) ||
      ...
      /* DUECA system.
        ...
        affected node.
      */
      E_CNF(getId() << "environment, illegal FIFO priority " << level[ii]);
      return false;
    }
...
void Environment::graphicRun()
{
  ...
  */
  I_SYS(getId() << " passing control to graphics lib");
  ...
  */
  I_SYS(getId() << " returned from graphics loop");

  // returned from the gui, and therefore also from possibly creation
  // and later joining of real-time threads. Now join the this thread
  ...
  case MultiThread:
    ...
    if (need_to_start_others) {
      ...
      if (TimeKeeper::readClock() > rt_start_time) {
        ...
        */
        I_SYS("Time to start real-time running");
        ...
        /* DUECA system.
          ...
          managers.
        */
        I_SYS(getId() << " starting activity managers");
        ...
        */
        I_SYS(getId() << " setting ticker to autonomous");
...
void Environment::readMod(const vstring& fname)
{
  ...
  if (ScriptInterpret::single()->
      readAdditionalModuleConf(fname)) {
    ...
    /* DUECA system.
       ...
       model configuration.
    */
    W_CNF(getId() << " error reading file " << fname);
  }
}
...
void Environment::waitAdditional(const TimeSpec& ts)
{
  if (ScriptInterpret::single()->modelCopied()) {
    ...
    while (create_cmd == Read) {
      ...
      */
      I_SYS(getId() << " no 0 thread waiting on script reading");
      create_control.wait();
    }
    ...
    */
    I_SYS(getId() << " no 0 thread continues");
    entity_manager->createEntityModules();
  }
}
...
#endif
{
  ...
  */
  I_SYS("Jettisoning everyone except crew members");
...
void Environment::proceed(int stage)
{
  ...
  */
  I_SYS("Popped up from script, stage=" << stage);
  ...
  case 1: {
    ...
    */
    I_SYS("Creating the node manager");
    ...
    /* DUECA system.
      ...
      initialization.
    */
    I_SYS("Completing dueca objects");
    ...
    command_interval_ticks = command_interval > Ticker::single()->getDT() ?
      int(round(command_interval / Ticker::single()->getTimeGranule())) :
      Ticker::single()->getBaseIncrement();
    command_lead_ticks = command_lead >= 0 ?
    ...
    if (sigaction(SIGINT, &act, NULL) == -1) {
      ...
      */
      W_SYS("Cannot specify Ctrl-c handler " << strerror(errno));
    }
    ...
    */
    I_SYS("Dueca nodes now complete");
    ...
    countdown = 10;
    while (!ScriptInterpret::single()->modelCopied() ||
           countdown--) {
      update();
    ...
    /* DUECA system.
       ...
       all nodes.
    */
    I_SYS("Real-time running, two seconds from now ....");
  }
  ...
  case 2:
	...
	if (mlcklim.rlim_cur != RLIM_INFINITY) {
       ...
       */
      I_SYS(getId() << " creating graphics thread with (e)uids: " <<
            geteuid() << ", " << getuid());
	...
	W_SYS("Cannot create graphics thread, trying with default stack size");
	...
	err = pthread_create(&activity0_thread, &thread_attrib,
      ...
      I_SYS("Script thread continues");
        ...
        if (create_cmd == Exit) {
          ...
          I_SYS(getId() << " joined script and graphics threads");
        }
        else {
          create_control.leaveTest();
        }
  ...
  case 3:
    ...
    if (create_cmd == Exit) {
      ...
      /* DUECA system.
        ...
        Script and graphics threads are combined again. */
      I_SYS("Joined script and graphics threads");
    }
    else {
      create_control.leaveTest();
    }
